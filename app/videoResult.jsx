...
import { detectVideo, detectVideoByHttp } from "@/utils/api";
...
const ResultPageImg = () => {
    ...
    const videoUri = searchParams.videoUri || "";
    const videoLink = searchParams.videoLink || "";
    const interval = searchParams.interval? parseFloat(searchParams.interval) : 2;
    ...
    useEffect(() => {
        if (newflag === 1) {
            ...
            //         setIsLoading(false);
            //     });
            console.log("videoUriiii: ",videoUri);
            if(videoUri){
                detectVideo(videoUri,interval)
                .then(response => {
                    try {
                        // const data = JSON.parse(response);
                        const data=response;
                        console.log("data",data);
                        setPercentage((data.toFixed(4))*100);
                        setIsLoading(false);
                    } catch (error) {
                        console.error('解析 JSON 出错:', error);
                        setIsLoading(false);
    ...
    const saveToLocal = async () => {
        try {
            const fileUri = FileSystem.documentDirectory + 'videoHistory.json';
            ...
            // 添加新数据
            const newData = { percentage, index: newIndex, time: currentTime, uri: videoUri, link: videoLink };
    ...
    const deleteRecord = () => {
        removeRecordByIndex(nowIndex,'videoHistory.json');
        deleteFileByPath(videoUri);// 同时记得删除图片啊
        router.back();
    }
                ...
                <ScrollView>
                    ...
                    {videoUri ? (
                        ...
                        <Card style={styles.linkCard}>
                            <Card.Content>
                                <Text style={styles.linkText}>视频链接：{videoLink}</Text>
                            </Card.Content>
                        </Card>
                    )}
                    ...
                    <TouchableOpacity style={styles.deleteButton} onPress={()=>deleteFile('videoHistory.json')}>
